...
export default function RecordStoragePage() {
  ...
  const [metadata, setMetadata] = useState(
    '{"warehouse":"Bodega Central 3","temperature":"22C","humidity":"60%","notes":"Stored after transport"}'
  );
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Failed to record storage"
      );
    } finally {
    ...
    <div>
      <div className="bg-black text-white py-12 px-4 border-b-4 border-black mb-12">
        <div className="max-w-7xl mx-auto">
          ...
          <h1 className="text-4xl font-black mb-2">Record Storage</h1>
          <p className="text-gray-300 font-medium">
            Register a storage event for a cacao batch in the logistics stage.
          </p>
        </div>
      </div>
        ...
        >
          ...
          <div>
            ...
            <input
              ...
              placeholder="e.g. Buenaventura Storage Hub"
              className="w-full px-4 py-3 text-black bg-gray-50 border-4 border-black rounded-lg focus:outline-none focus:ring-2 focus:ring-gray-400 focus:bg-white"
              required
            />
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              rows={6}
              placeholder='{"warehouse":"Bodega Central 3","temperature":"22C","humidity":"60%","notes":"Stored after transport"}'
              className="w-full px-4 py-3 text-black bg-gray-50 border-4 border-black rounded-lg focus:outline-none focus:ring-2 focus:ring-gray-400 focus:bg-white"
              required
            />
            <p className="text-xs text-gray-600 font-semibold mt-2">
              Suggested fields: warehouse, temperature, humidity, storage_time,
              notes.
            </p>
          </div>
          ...
          <div className="flex gap-4 pt-4">
            ...
            >
              {isSubmitting ? "⏳ Recording..." : "✓ Record Storage"}
            </button>
