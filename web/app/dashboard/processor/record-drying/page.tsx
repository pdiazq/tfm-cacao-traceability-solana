...
export default function RecordDryingPage() {
  ...
  const [metadata, setMetadata] = useState(
    '{"duration_hours":48,"temperature":"38C","humidity":"12%","notes":"Sun drying completed"}'
  );
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Failed to record drying"
      );
    } finally {
    ...
    <div>
      <div className="bg-black text-white py-12 px-4 border-b-4 border-black mb-12">
        <div className="max-w-7xl mx-auto">
          ...
          <h1 className="text-4xl font-black mb-2">Record Drying</h1>
          <p className="text-gray-300 font-medium">
            Register a drying event for a cacao batch.
          </p>
        </div>
      </div>
        ...
        >
          ...
          <div>
            ...
            <textarea
              ...
              rows={6}
              placeholder='{"duration_hours":48,"temperature":"38C","humidity":"12%","notes":"Sun drying completed"}'
              className="w-full px-4 py-3 text-black bg-gray-50 border-4 border-black rounded-lg focus:outline-none focus:ring-2 focus:ring-gray-400 focus:bg-white"
              required
            />
            <p className="text-xs text-gray-600 font-semibold mt-2">
              Suggested fields: duration_hours, temperature, humidity, moisture, notes.
            </p>
          </div>
          ...
          <div className="flex gap-4 pt-4">
            ...
            >
              {isSubmitting ? "⏳ Recording..." : "✓ Record Drying"}
            </button>
